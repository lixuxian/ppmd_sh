...
int ppmd_compress(const char *inputfile, const char *outputfile)
{
  FILE *f = fopen(inputfile, "rb"); if (f == 0) return 2;
  FILE *g = fopen(outputfile, "wb"); if (g == 0) return 3;

  uint f_DEC = 0; // 1解压， 0压缩

  // {
  //   int i;
  //   for( i=0; i<Min<int>(argc-4,DIM(pmd_args1)); i++ ) {
  //     char* p = argv[4+i]; p += (p[0]=='-');
  //     pmd_args1[i]=atoi(p);  // 获取最后几个参数
  //   }
  // }
...
int ppmd_decompress(const char *inputfile, const char *outputfile)
{
  FILE *f = fopen(inputfile, "rb"); if (f == 0) return 2;
  FILE *g = fopen(outputfile, "wb"); if (g == 0) return 3;

  uint f_DEC = 1; // 1解压， 0压缩
...
int ppmd_api( int argc, char** argv ) {
